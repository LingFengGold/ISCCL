...
def run(args):
    ...
    # model
    torch.cuda.set_device(f"cuda:{args.gpuid}")
    ...
    model = SCCLBert(args, bert, tokenizer, cluster_centers=cluster_centers) 
    model = model.cuda()
...
def get_args(argv):
    ...
    parser.add_argument('--gpuid', type=int, default=-1, help="")
