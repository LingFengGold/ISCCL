...
class SCCLBert(nn.Module):
    def __init__(self, args, bert_model, tokenizer, cluster_centers=None):
        ...
        self.cluster_head = nn.Sequential(
            nn.Linear(self.emb_size, self.emb_size),
            nn.ReLU(inplace=True),
            nn.Linear(self.emb_size, args.num_classes))
    ...
    def contrast_logits_cluster(self, embd1, embd2=None):
        feat1 = F.normalize(self.cluster_head(embd1).T, dim=1)
        if embd2 != None:
            feat2 = F.normalize(self.cluster_head(embd2).T, dim=1)
            return feat1, feat2
        else: 
            return feat1
